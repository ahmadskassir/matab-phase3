...
if ($clinic_id) {
    ...
    $sql = "SELECT * FROM clinic_appointments WHERE clinic_id = '$clinic_id' AND status = 'available' ORDER BY appointment_date, start_time";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['appointment_id']) && isset($_SESSION['user_id'])) {
    ...
    $sql = "SELECT * FROM clinic_appointments WHERE id = '$appointment_id' AND status = 'available'";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        $appointment = $result->fetch_assoc();
        $booking_date = $appointment['appointment_date'] . ' ' . $appointment['start_time'];
        $booking_type = 'clinic';
        $target_name = $clinic['name'];
        ...
        if ($conn->query($sql)) {
            ...
            $sql = "UPDATE clinic_appointments SET status = 'booked' WHERE id = '$appointment_id'";
            $conn->query($sql);
...
<body>
    <div class="details-container">
        ...
        if ($clinic) {
            echo '
                <img src="' . ($clinic['image'] ?: 'placeholder.jpg') . '" alt="' . $clinic['name'] . '" class="clinic-img">
                <h2>' . $clinic['name'] . '</h2>
                <p><strong>توضیحات:</strong> ' . ($clinic['description'] ?: 'بدون توضیحات') . '</p>
                <p><strong>آدرس:</strong> ' . $clinic['address'] . '</p>
            ';
        } else {
            echo '<p>کلینیک یافت نشد</p>';
        }
        ?>
        ...
        <div class="booking-section">
            ...
            <?php endif; ?>
            <?php if ($clinic && !empty($appointments)): ?>
                <form method="POST">
        ...
        <a href="clinics.php"><button class="back-btn">بازگشت به لیست کلینیک‌ها</button></a>
    </div>
</body>
</html>
