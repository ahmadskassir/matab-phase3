...
$sql = "SELECT id FROM imaging_centers";
...
foreach ($imaging_ids as $imaging_id) {
    foreach ($days as $day) {
        ...
        foreach ($slots as $start_time) {
            ...
            $sql = "INSERT INTO imaging_appointments (imaging_id, appointment_date, start_time, end_time, status) 
                    VALUES ('$imaging_id', '$date', '$start_time', '$end_time', 'available')";
            $conn->query($sql);
        }
    }
}
