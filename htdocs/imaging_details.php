...
$imaging_id = isset($_GET['id']) ? $_GET['id'] : null;
$imaging = null;
$appointments = [];

if ($imaging_id) {
   
    $sql = "SELECT * FROM imaging_centers WHERE id = '$imaging_id'";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        $imaging = $result->fetch_assoc();
    }

    
    $sql = "SELECT * FROM imaging_appointments WHERE imaging_id = '$imaging_id' AND status = 'available' ORDER BY appointment_date, start_time";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['appointment_id']) && isset($_SESSION['user_id'])) {
    ...
    $sql = "SELECT * FROM imaging_appointments WHERE id = '$appointment_id' AND status = 'available'";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        $appointment = $result->fetch_assoc();
        $booking_date = $appointment['appointment_date'] . ' ' . $appointment['start_time'];
        $booking_type = 'imaging';
        $target_name = $imaging['name'];
        ...
        if ($conn->query($sql)) {
            ...
            $sql = "UPDATE imaging_appointments SET status = 'booked' WHERE id = '$appointment_id'";
            $conn->query($sql);
...
<head>
    ...
    <title>مطب - جزئیات مرکز تصویربرداری</title>
    <style>
        body {
...
<body>
    <div class="details-container">
        ...
        if ($imaging) {
            echo '
                <img src="' . ($imaging['image'] ?: 'placeholder.jpg') . '" alt="' . $imaging['name'] . '" class="imaging-img">
                <h2>' . $imaging['name'] . '</h2>
                <p><strong>توضیحات:</strong> ' . ($imaging['description'] ?: 'بدون توضیحات') . '</p>
                <p><strong>آدرس:</strong> ' . $imaging['address'] . '</p>
            ';
        } else {
            echo '<p>مرکز تصویربرداری یافت نشد</p>';
        }
        ?>
        ...
        <div class="booking-section">
            ...
            <?php endif; ?>
            <?php if ($imaging && !empty($appointments)): ?>
                <form method="POST">
        ...
        <a href="imaging.php"><button class="back-btn">بازگشت به لیست مراکز تصویربرداری</button></a>
    </div>
</body>
</html>
